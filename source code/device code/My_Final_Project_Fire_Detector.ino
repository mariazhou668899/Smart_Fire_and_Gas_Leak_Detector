/*
 * Introduction:
 * - This is a smart fire and gas leak detector and alarm system
 * - by using Azure IoT Central and Espressif ESP32.
 * - It uses Azure Embedded SDK for C to help interact with Azure IoT.
 *
 * To connect and work with Azure IoT Hub it needs an MQTT client, connecting, subscribing
 * and publishing to specific topics to use the messaging features of the hub.
 * azure-sdk-for-c is an MQTT client support library, helping composing and parsing the
 ...
 * - Replace template Model id to implement personal defined custom template.
 * - AZURE_PNP_MODEL_ID "XXXX"
 *
 * iot_configs.h is to fill the specific information which will be used for connecting to Azure IoT services
 * - IOT_CONFIG_WIFI_SSID "XXXX"
 * - IOT_CONFIG_WIFI_PASSWORD "XXXX"
 * - DPS_ID_SCOPE "XXXX"
 * - IOT_CONFIG_DEVICE_ID "XXXX"
 * - IOT_CONFIG_DEVICE_KEY "XXXX"
...
/* --- define DHT22 sensor --- */
#define DHTPIN 4
#define DHTTYPE DHT22        // DHT sentor type: DHT 11

/* --- define MQ2 sensor --- */
int Gas = 36;                // Pin 36 of ESP32
int Note_B4 = 500;           // tone of buzzer for gas higher limit value

/* --- define buzzer --- */
int Buzzer = 19;             // Pin 19 of ESP32
int Note_E4 = 1500;          // tone of buzzer for temperature higher limit value

int Delay = 500;             // Set the buzzer pause time

/* --- Define Led for indicating if the buzzer monitoring on or off --- */
int Led = 17;                // Pin 17 of ESP32

/* --- define temperature limit value for alarm --- */
float alarm_temperature;     // Get temperature value changed by IoT Central Command

/* --- define gas / smoke limit value for alarm --- */
float alarm_gas;    // Get gas value changed by IoT Central Command

/* --- define buzzer and led on or off --- */
bool buzzer_status = false; // Get the setting buzzer monitoring status from IoT Central Command
...
/*
 * These functions are used by Azure IoT to interact with whatever MQTT client, here uses Espressif's ESP MQTT. 
 */
...
void setup()
{
  ...
  dht.begin();  // DHT22 starts working
  pinMode(Gas, INPUT);
...
void loop()
{
  ...
  // Convert the raw analog value to a float
  float gas = static_cast<float>(sensorValue);
