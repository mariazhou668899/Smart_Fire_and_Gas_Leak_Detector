...
#define AZURE_PNP_MODEL_ID "XXXXXXXX"    // Replace it with your own

//templeate information
...
// Define the command name for setting temperature alarm
static az_span COMMAND_NAME_SET_TEMP_ALARM = AZ_SPAN_FROM_STR("SetTemperatureAlarm");
float set_alarm_temperature = 130;

// Define the command name for setting gas alarm
static az_span COMMAND_NAME_SET_GAS_ALARM = AZ_SPAN_FROM_STR("SetGasAlarm");
float set_alarm_gas = 900;

// Define the command name for turn on buzzer monitoring
static az_span COMMAND_NAME_TOGGLE_BUZZER = AZ_SPAN_FROM_STR("TriggerBuzzer");
static bool buzzer_status = false;
    ...
    float gas)
{
  ...
  EXIT_IF_AZ_FAILED(
      rc, RESULT_ERROR, "Failed adding gas property value to telemetry payload. ");
